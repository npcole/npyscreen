...
class ActionFormV2(fmForm.FormBaseNew):
    ...
    def pre_edit_loop(self):
        # these next two lines do not work as the author expected...
        self._widgets__.sort(key=operator.attrgetter('relx'))  # is overridden by next line - why do it?
        self._widgets__.sort(key=operator.attrgetter('rely'))
        # update the _widgets_by_id also
        for w in self._widgets__:
            self._widgets_by_id[self._widgets__.index(w)] = weakref.proxy(w)
