...
class TextfieldBase(widget.Widget):
    ...
    def print_cursor(self):
        ...
        if self.do_colors():
            self.parent.curses_pad.addstr(self.rely,
                                          ...
                                          char_under_cur, self.parent.theme_manager.findPair(self, 'CURSOR_INVERSE'))
        else:
            self.parent.curses_pad.addstr(self.rely,
                                          self.cursor_position - self.begin_at + self.relx + self.left_margin,
                                          char_under_cur, curses.A_STANDOUT)
    ...
    def print_cursor_pre_unicode(self):
        ...
        self.parent.curses_pad.addstr(self.rely, self.cursor_position - self.begin_at + self.relx + self.left_margin,
                                      char_under_cur, curses.A_STANDOUT)
    ...
    def _print_pre_unicode(self):
        ...
        if self.syntax_highlighting:
            ...
            if self.show_bold and self.color == 'DEFAULT':
                coltofind = 'BOLD'
            if self.show_bold:
                self.parent.curses_pad.addstr(self.rely, self.relx + self.left_margin, string_to_print[
                                                                                       self.begin_at:self.maximum_string_length + self.begin_at - self.left_margin],
...
class FixedText(TextfieldBase):
    ...
    def update(self, clear=True, cursor=False):
...
class Numericfield(Textfield):
    """a Textfield that accepts only numeric input"""
