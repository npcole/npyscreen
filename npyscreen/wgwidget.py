...
big a given widget is ... use .height and .width instead"""
    ...
    def set_size(self):
        ...
        if self.height == RAISEERROR or self.width == RAISEERROR:
            ...
            raise NotEnoughSpaceForWidget("Not enough space: max y and x = %s , %s. Height and Width = %s , %s " % (
            my, mx, self.height, self.width))  # unsafe. Need to add error here.
    ...
    def clear(self, usechar=' '):
        ...
        for y in range(self.height):
            ...
            # Use this instead
            if self.do_colors():
                self.parent.curses_pad.addstr(self.rely + y, self.relx, usechar * (self.width),
                                              self.parent.theme_manager.findPair(self,
                                                                                 self.parent.color))  # used to be width + 1
            else:
